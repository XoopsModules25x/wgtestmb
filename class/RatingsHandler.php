...
class RatingsHandler extends \XoopsPersistableObjectHandler
{
    ...
    {
        parent::__construct($db, 'wgtestmb_ratings', Ratings::class, 'rate_id', 'rate_itemid');
    }
    ...
    public function getItemRating($itemid = 0, $source = 0)
    {
        ...
        if (Constants::RATING_5STARS === (int)$helper->getConfig('ratingbars')
            ...
            if (1 == $count) {
                $text      = \str_replace('%c', $ItemRating['avg_rate_value'], \_MA_WGTESTMB_RATING_CURRENT_1);
                $shorttext = \str_replace('%c', $ItemRating['avg_rate_value'], \_MA_WGTESTMB_RATING_CURRENT_SHORT_1);
            } else {
                $text      = \str_replace('%c', $ItemRating['avg_rate_value'], \_MA_WGTESTMB_RATING_CURRENT_X);
                $shorttext = \str_replace('%c', $ItemRating['avg_rate_value'], \_MA_WGTESTMB_RATING_CURRENT_SHORT_X);
            }
