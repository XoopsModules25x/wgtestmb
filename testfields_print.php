...
// Verify permissions
if (!$grouppermHandler->checkRight('wgtestmb_view', $tfId->getVar('tf_id'), $groups, $GLOBALS['xoopsModule']->getVar('mid'))) {
    \redirect_header(\WGTESTMB_URL . '/index.php', 3, \_NOPERM);
    exit();
}
$testfieldList = $testfields->getValuesTestfields();
$GLOBALS['xoopsTpl']->append('testfields_list', $testfieldList);

$GLOBALS['xoopsTpl']->assign('xoops_sitename', $GLOBALS['xoopsConfig']['sitename']);
$GLOBALS['xoopsTpl']->assign('xoops_pagetitle', \strip_tags($testfields->getVar('tf_text') . ' - ' . \_MA_WGTESTMB_PRINT . ' - ' . $GLOBALS['xoopsModule']->getVar('name')));
